...
import type { CourseType, TastePreference } from '@/app/page'; // Import shared types
...
const ProCourseSchema = z.object({
    ...
    ingredients: z.string().describe("A clearly formatted list (e.g., using markdown) of ALL ingredients required for this specific course recipe, including quantities suitable for the number of guests. Differentiate between user-provided and additional ingredients."),
...
const GenerateProMenuOutputSchema = z.object({
  ...
  chefNotes: z.string().optional().describe('Optional overall notes from the chef regarding the menu, potential challenges, substitutions, or why generation might have failed (e.g., "Insufficient ingredients for requested courses", "Difficulty matching theme with ingredients").')
...
**Chef's Task:**
1.  **Analyze Core Ingredients:** Review the user's {{{ingredients}}}. Determine if they are sufficient *foundation* for the requested number of courses ({{courses.length}} courses for {{numGuests}} guests).
2.  **Failure Condition:** If the core ingredients are fundamentally insufficient or completely unsuitable for the requested courses/theme/preferences (even with reasonable additions), **indicate failure**:
    *   Set 'menuTitle' to indicate failure (e.g., "Menu Generation Failed" or its translation).
    *   Keep 'courses' as an empty array ([]).
    *   Explain the reason clearly in 'chefNotes' (e.g., "Cannot create {{courses.length}} distinct courses for {{numGuests}} guests with only {{{ingredients}}}.", "Ingredients conflict strongly with requested '{{{tastePreference}}}' preference.").
    *   **STOP HERE if failure is indicated.**
3.  **Menu Planning (If Feasible):**
    *   Devise a *cohesive* menu featuring the requested courses ({{#each courses}}{{{this}}}{{#unless @last}}, {{/unless}}{{/each}}).
    ...
    *   For EACH requested course:
        ...
        *   **Prioritize using the user's {{{ingredients}}}** across the different courses where sensible.
        *   Determine **ALL** necessary ingredients for *that specific recipe* for *{{numGuests}} guests*. Format this in the 'ingredients' field for the course using Markdown (e.g., "**Provided:**\\n- Chicken (approx. X lbs)\\n**Additional:**\\n- Onion (2 large)\\n- Olive Oil (1/4 cup)..."). Clearly state which ingredients are assumed to be provided by the user and which are additional suggestions. Include estimated quantities appropriate for {{numGuests}}.
...
4.  **Chef Notes:** Add optional 'chefNotes' for overall comments, potential substitutions across the menu, timing suggestions, or challenges encountered (e.g., "Starter and main share similar flavor profiles due to ingredient limitations.", "Dessert is simple to allow focus on main course.").

**Output Format:** Respond *strictly* following the output schema: {menuTitle: string, eventTheme: string, numGuests: number, courses: [{type: string, recipeName: string, ingredients: string, instructions: string}], chefNotes?: string}. Ensure all text is in the requested language ({{#if language}}{{language}}{{else}}English{{/if}}). 'courses' MUST be an array, even if empty on failure.
`,
});
  ...
  async (input) => {
    ...
    try {
      ...
      if (output) {
         ...
         // Post-processing: If AI indicates failure in title but provides no notes, add a default note.
         if ((output.menuTitle.includes("Failed") || output.menuTitle.includes("Error")) && !output.chefNotes) {
         ...
         else if (!(output.menuTitle.includes("Failed") || output.menuTitle.includes("Error")) && output.courses.length === 0) {
...
function getErrorMessages(lang: string) {
    const messages: { [key: string]: any } = {
       ...
       "fr": {
            ...
            flowError: { aiErrorTitle: "Erreur IA : Pas de Réponse", aiErrorNotes: "Le chef IA n'a pas renvoyé de menu. Veuillez réessayer.", unexpectedErrorNotes: "Une erreur inattendue s'est produite lors de la génération du menu.", busyNotes: "Le chef IA est occupé à planifier des menus ! Veuillez réessayer.", busyTitle: "IA Occupée", configErrorNotes: "Il semble y avoir un problème avec la configuration de l'IA.", configErrorTitle: "Erreur de Configuration", aiErrorGenericNotes: "Erreur IA : {message}", aiErrorGenericTitle: "Erreur IA", defaultFailTitle: "Échec de la Génération du Menu", defaultFailNotesInsufficient: "Impossible de générer le menu. Les ingrédients sont peut-être insuffisants ou inadaptés." }
        },
       "de": {
